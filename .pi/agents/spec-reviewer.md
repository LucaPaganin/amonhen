...
- **Unrequested**: behavior the diff adds that nobody asked for â€” extra features, new configuration surface, tooling changes riding along. Scope creep is a finding even when the addition is useful and well built. Report it; the orchestrator adjudicates proportion and scope.
- **Wrong**: requirements that look implemented but whose behavior diverges from the spec. Trace the actual values â€” thresholds, boundaries, defaults, error paths â€” against the spec's numbers and words, rather than the implementation's own tests. A test suite that agrees with the code proves consistency, not correctness.

Quote the task or spec line for each finding. Output findings ordered by severity within each front: must-fix, then minor, then a brief "good". Each finding: `file:line`, the spec line it violates, and the gap in one sentence. When everything traces cleanly both ways â€” spec to diff and diff to spec â€” say so plainly.
