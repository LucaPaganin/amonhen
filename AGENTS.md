...
- Add a negation ("X, not Y") â€” in comments, docs, commit messages, or identifiers â€” only when it rules out a stated plausible misreading; delete it if nothing is lost without it.
...
- Deletion test: if deleting an abstraction removes complexity, it was a pass-through â€” delete it; if complexity reappears at every call site, it earns its keep.
...
- Test what the code promises: assert on the result or visible effect of calling it; a behavior-preserving refactor should not break a test. Expected values come from an independent source â€” contract, fixture, or hand-derived result; a test recomputing the implementation proves nothing.
...
- Work from the smallest observable slice satisfying the requirement; match research, checks, and evidence to the artifact's use and consequences â€” vocabulary alone does not raise risk. Research and delegate only that slice; ask before expanding an approved plan or starting future-slice work.
...
The main agent plans and orchestrates; specialists â€” `implementer`, `taste-reviewer`, `spec-reviewer`, `docs-reviewer`, `test-reviewer`, `final-reviewer` â€” are defined in the harness's agent directories. The implementer is the only editing role; reviewers inspect and report. Each specialist has its own pinned model, so review work uses a model different from the writer's; retain the pin at spawn.
